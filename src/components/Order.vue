      ...
      <div class="two_bg">
          <div class="two_t">
            ...
            <table border="0" class="car_tab" style="width:1110px;" cellspacing="0" cellpadding="0">
              ...
              <tr v-for="value in order.data">
                <td>
                    <div class="c_s_img"><img src="/static/images/c_1.jpg" width="73" height="73" /></div>
                    {{value[0].name}}
                </td>
                <td align="center">{{value[0].attr_name}}</td>
                <td align="center">{{value[0].number}}</td>
                <td align="center" style="color:#ff4e00;">￥{{value[0].number*value[0].price}}</td>
            ...
            <table border="0" style="width:900px; margin-top:20px;" cellspacing="0" cellpadding="0">
              <tr height="70">
                <td align="right">
                  <b style="font-size:14px;">应付款金额：<span style="font-size:22px; color:#ff4e00;">￥{{prices}}</span></b>
                </td>
              </tr>
              <tr height="70">
                <td align="right"><a href="#" @click="orders"><img src="/static/images/btn_sure.gif" /></a></td>
              </tr>
            </table>
...
<script>
  ...
  mounted(){
    axios.post('http://localhost/03kj/blog/public/api/orders',{
    ...
    .then(response=>{
      ...
      var number=0
      this.order.data.forEach(function (val){
        number += val[0].price * val[0].number
      })
      console.log(number)
      this.prices=number
    })
  ...
  methods:{
    orders(){
      axios.post('http://localhost/03kj/blog/public/api/orders_add',{
      token:localStorage.getItem("token"),
      number:localStorage.getItem("order"),
    })
     .then(response=>{
      var token=localStorage.getItem("token")
      location.href="http://localhost/03kj/blog/public/api/pay/index?token="+token+"&oid="+response.data[0]+"&a="+response.data[1]

...
<div class="i_bg">
    ...
    <div class="content">
        ...
        <div class="pro_des">
            ...
            <div class="des_price">
            	本店价格：<b v-for="(v,key) in price">{{v.price}}</b><br/>
                消费积分：<span>28R</span>
            </div>
            <div class="des_choice" v-for="(v,key) in msg.data">
                  <span class="fl" >{{key}}：</span>
                  <label>
                    <span v-for="timk in v">
                    <input id="a1" type="radio" v-bind:value="timk[1]" v-bind:name="key" v-on:click="getradio">{{timk[0]}}
                      </span>
                  </label>
              </div>
            <div class="des_share">
            ...
            <div class="des_join">
                ...
                <span class="fl"><a @click="shoppingcar"><img src="/static/images/j_car.png" /></a></span>
            </div>            
        </div>    
...
export default {
  ...
  mounted () {
    axios.post('http://localhost/03kj/blog/public/api/goods',{
      id:this.$route.query.aId
    })
      .then(response =>(this.msg=response.data
        )),
      axios.post('http://localhost/03kj/blog/public/api/user_name',{
        token:localStorage.getItem("token"),
      name: this.name,
    })
      .then(response=> {
  ...
  methods:{
    getradio(){
      ...
      $.each(this.msg.data,function(key,val){
        ...
        //当input标签里面内容单个被选中时将提示undefined，只有所有标签里的内容全部被选中时才会触发ajax将所获取的值传过去
      ...
      if (this.isajax==1) {
        //当都被选中时取出标签中的内容并循环进一个新的数组中
         var id=''
         var attr=''
        $.each(this.msg.data,function (keys,value) {
          id=id+","+ $("input[name='"+keys+"']:checked").val()
          attr=attr+","+ $("input[name='"+keys+"']:checked").attr('name')+":"+$("input[name='"+keys+"']:checked").attr
        })
        //获取价格的
        // console.log('ajax')
        axios.post('http://localhost/03kj/blog/public/api/goodshp',{
          id:id,
          ...
          .then(response =>{
            ...
            _this.prices=response.data[0].price
          })
      }
      this.isajax=1
      // console.log(id)
    ...
    shoppingcar () {
      ...
      axios.post('http://localhost/03kj/blog/public/api/car',{
        num:num,
        goods_id:this.price[0].id,
        user_id:this.user_name[0].id,
        token: localStorage.getItem("token"),
      })
        .then(response =>{
